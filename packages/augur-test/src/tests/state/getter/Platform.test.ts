...
describe('State API :: get-platform-activity-stats :: ', () => {
  ...
  test('getPlatformActivityStats', async () => {
    ...
    let errorMessage = '';
    try {
      const markets: MarketInfo[] = await api.route('getPlatformActivityStats', {
    ...
    let stats = await api.route('getPlatformActivityStats', {
    ...
    stats = await api.route('getPlatformActivityStats', {
      universe: universe.address,
      account: ACCOUNTS[0].publicKey,
    });
